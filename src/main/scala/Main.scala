    ...
    DB.get foreach {
      case (db, close) =>
        ...
        def withAnalysis(gs: Seq[Game.WithInitialFen]): Future[Seq[Analysed]] =
          db.analysisColl.find(
            BSONDocument("_id" -> BSONDocument("$in" -> gs.filter(_.game.metadata.analysed).map(_.game.id)))
          ).cursor[Analysis](readPreference = ReadPreference.secondary).collect[List]() map { as =>
              gs.map { g =>
        ...
        def toPgn(ws: Seq[WithUsers]): Future[Seq[Pgn]] = Future(ws map {
        ...
        def pgnSink: Sink[Seq[Pgn], Future[IOResult]] =
          Flow[Seq[Pgn]]
            .map { pgns =>
              ...
              // 1. e4 { [%eval 0.17] [%clk 0:00:30] }
              val str = pgns.map(_.toString).mkString("\n\n").replace("] } { [", "] [")
              ByteString(s"$str\n\n")
            }
