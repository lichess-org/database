...
object BinaryFormat:
  ...
  final class clock(start: Timestamp):
    ...
    private def writeTimer(timer: Timestamp) =
       ...
       */
      val nonZero = java.lang.Math.max(centis, 1)
      writeInt(nonZero)
  ...
  object clock:
    def apply(start: java.time.Instant) = new clock(Timestamp(start.toMillis))
  ...
  object piece:
    ...
    val standard = write(Board.init(chess.variant.Standard, White).pieces)
