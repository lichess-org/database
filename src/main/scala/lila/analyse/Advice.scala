...
sealed trait Advice {
  ...
  def makeComment(withEval: Boolean, withBestMove: Boolean): String =
    evalComment.filter(_ => withEval).fold("") { c => s"($c) " } +
      (this match {
        case MateAdvice(seq, _, _, _) => seq.desc
        case CpAdvice(judgment, _, _) => judgment.toString
      }) + "." + {
        info.variation.headOption.filter(_ => withBestMove).fold("") { move => s" Best move was $move." }
      }

  def evalComment: Option[String] = Some {
